 ...
 */
    ...
    public:
        ...
        protected:
            ...
            Node(const Element& key = Element(), Node* left = nullptr, Node* right = nullptr, Node* parent = nullptr,
                 Color col = BLACK) : _key(key), _left(left), _right(right), _parent(parent), _color(col)
            {
    ...
    protected:
        ...
        void fixUpAfterDeletion(Node* nd);

        Color getColor(Node* nd) { return nd == nullptr ? BLACK : nd->_color; }

        Node* getParent(Node* nd) { return nd == nullptr ? nullptr : nd->_parent; }

        Node* getLeft(Node* nd) { return nd == nullptr ? nullptr : nd->_left; }

        Node* getRight(Node* nd) { return nd == nullptr ? nullptr : nd->_right; }

        void setColor(Node* nd, Color color) { if(nd != nullptr) nd->_color = color; }
